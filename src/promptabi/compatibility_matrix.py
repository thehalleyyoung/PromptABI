...
CHECK_RULE_IDS: dict[str, tuple[str, ...]] = {
    ...
    "prompt-pack-contracts": (
        ...
        "prompt-pack-composition-context-role-unsupported",
        "prompt-pack-composition-rag-unbounded",
        "prompt-pack-composition-required-region-not-must-survive",
        "prompt-pack-composition-required-region-truncated",
        "prompt-pack-composition-required-region-undeclared",
        "prompt-pack-composition-verified",
        "prompt-pack-empty",
        "prompt-pack-model-family-unsupported",
        "prompt-pack-stop-missing",
        "prompt-pack-template-role-mismatch",
        "prompt-pack-template-tokenizer-abstained",
        "prompt-pack-template-tokenizer-control-token",
        "prompt-pack-template-tokenizer-verified",
        "prompt-pack-tool-missing",
        "prompt-pack-verified",
    ),
    ...
    "evaluation-harness-contracts": (
        ...
        "evaluation-harness-tokenizer-unpinned",
...
def _surfaces_for_check(check_name: str, artifact_kinds: tuple[ArtifactKind, ...]) -> tuple[CompatibilitySurface, ...]:
    explicit: dict[str, tuple[CompatibilitySurface, ...]] = {
        ...
        "training-drift": (
            _surface("training", "training-corpus-metadata", ArtifactKind.TRAINING_MANIFEST, "bounded", "training corpus baseline metadata is compared against model-card facts"),
            _surface("training", "model-card-drift", ArtifactKind.TRAINING_MANIFEST, "bounded", "model-card tokenizer/template corpus declarations must match corpus metadata"),
            _surface("training", "tokenizer-template-drift", ArtifactKind.TRAINING_MANIFEST, "bounded", "corpus tokenizer/template pins must match preparation, training, serving, and lockfile pins"),
            _surface("training", "serving-lockfile-drift", ArtifactKind.TRAINING_MANIFEST, "bounded", "serving lockfile metadata must match the corpus baseline"),
        ),
        "training-invalid-interface": (
            _surface("training", "impossible-roles", ArtifactKind.TRAINING_MANIFEST, "bounded", "declared training role usages must fit the structural role set or explicit custom-role allowlist"),
            _surface("training", "malformed-tool-calls", ArtifactKind.TRAINING_MANIFEST, "bounded", "finite tool-call examples exported by data preparation must parse under the declared tool-call contract"),
        ...
        "evaluation-harness-contracts": (
            ...
            _surface("evaluation", "multi-turn-history", ArtifactKind.EVALUATION_HARNESS, "bounded", "message history truncation, system-prompt survival, role alternation, and tool availability are checked for finite multi-turn benchmark transcripts"),
            ...
            _surface("evaluation", "benchmark-tokenizer-drift", ArtifactKind.EVALUATION_HARNESS, "bounded", "HELM-style, lm-eval-style, custom, and provider-hosted benchmark tokenizer pins are compared against local tokenizer configs or explicitly abstained"),
            *_provider_surfaces(),
            *_tokenizer_surfaces(),
        ),
...
def _notes_for_check(check_name: str) -> str:
    return {
        ...
        "evaluation-harness-contracts": "checks finite benchmark prompt variables, few-shot and multi-turn history contracts, answer-parser schema assumptions, tokenizer/template pins, benchmark-tokenizer drift, provider/model metadata, and stop policies before evaluation scores are trusted",
