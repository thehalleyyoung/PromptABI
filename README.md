...
| **Tools + providers** | OpenAI, Anthropic, MCP, LangChain, Pydantic, TypeScript-style, vLLM, llama.cpp, LiteLLM, Gemini, Bedrock, Groq, Together, and Ollama serialization drift, adapter-chain envelope preservation, plus replayable provider-conformance test vectors, error-envelope/context-window/stop-policy equivalence, streaming chunk-order, tool-call cancellation, migration dry-runs, and benchmark-drift deployment gates |
| **Prompt packs + contracts** | reusable templates and assume/guarantee contracts with compositional tokenizer/template/role/tool/stop/model-family/RAG/truncation guarantees, inherited proof-obligation lineage, locks, monotone extension certificates, semver/advisory gates, schema certification, signed third-party certification, transitive-lockfile registries, transparency logs, policy inheritance, trust tiers, signed offline mirrors, and deprecation/LTS lifecycles |
...
checks against real tokenizer/template libraries, an executable metatheory that
mechanizes role non-forgeability, noninterference, schema soundness/completeness,
and ten more theorems by exhaustive bounded enumeration, and a Z3-backed finite
...
promptabi prompt-pack monotonicity --config examples/prompt-packs/promptabi.json --baseline-lockfile /tmp/prompt-pack.lock.json
...
promptabi solver minimal-unsat-cores --format text
