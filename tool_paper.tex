...
privacy-preserving fingerprints. The implementation is validated by property
tests, differential tests against real tokenizer/template libraries, replayable
solver certificates, and public bug replays that traverse exact pinned
production-code excerpts rather than only synthetic reductions.
\end{abstract}
...
\begin{enumerate}
  ...
  \item an evidence suite built from property tests, differential tests against
  real libraries, replayable solver obligations, and public production-code bug
  replays.
\end{enumerate}
...
PromptABI's evidence strategy is layered.

\paragraph{Executable examples.}
Each major checker has examples that run through the public CLI or API. These
examples are not screenshots; tests execute representative commands and inspect
the resulting diagnostics.

\paragraph{Property and regression tests.}
Generated and hand-written tests exercise safe, unsafe, ambiguous, unsupported,
satisfiable, unsatisfiable, and abstaining cases. Snapshot tests lock diagnostic
shape so integration users can depend on stable machine-readable output.

\paragraph{Differential validation against real libraries.}
Tokenizer and template abstractions are compared against real Hugging Face,
SentencePiece, \texttt{tiktoken}, grammar, and parser behavior where those
libraries define the deployed semantics.

\paragraph{Public bug replay.}
PromptABI includes a real-world bug corpus covering public reports in
llama.cpp, vLLM, and Hugging Face Transformers. The benchmarked template and
tool-parser cases now replay from exact pinned production-code excerpts. The
Phi-style role-boundary case traverses the upstream chat template bytes before
finding forged role markers. The Qwen3 XML tool-parser case extracts stop
delimiters from the upstream parser source before proving that valid tool
arguments can be truncated. Other cases retain minimized local payload models
when the payload, not the upstream parser/template code, is the bounded object
under test.

\paragraph{Replayable solver certificates.}
SMT-backed findings can be reduced to solver-replay JSON objects. These rerun
the relevant finite obligation without requiring prompts, datasets, provider
credentials, or full repository state.

\section{Evaluation Methodology}

A PLDI-style evaluation of PromptABI asks four questions.

\begin{enumerate}
  \item \textbf{Bug finding:} does the tool catch known prompt-interface
  failures before inference?
  \item \textbf{Precision of evidence:} are reports actionable, replayable, and
  tied to the responsible artifact?
  \item \textbf{Coverage boundary:} does the tool clearly separate verified
  fragments from abstentions?
  \item \textbf{Integration cost:} can developers run it in ordinary CPU-only
  workflows without model weights or secrets?
\end{enumerate}

PromptABI answers these questions with its public bug replay suite, structured
schema and provider fixture suites, tokenizer and grammar conformance suites,
training/evaluation fixture packs, release gates, and comparative studies. The
important point is not a single aggregate number. The useful evidence is the
combination of replayed bugs, minimized witnesses, differential agreement,
explicit abstentions, and deterministic CI behavior.
...
its diagnostics carry executable witnesses and because its real-bug evidence now
includes exact production-code traversal, not only synthetic fixtures of known
failures.
