...
and structured-output boundaries. The search identified ten PromptABI-class
records. A record was admitted to the evaluation only if the issue or PR named an
upstream production artifact and PromptABI could recover the relevant boundary
from exact pinned source bytes. Six records meet that bar in the current corpus.
Records that require hand-reduced payloads, live model sampling, or a verifier
family not yet implemented are excluded from the counted evaluation.

Table~\ref{tab:eval} lists the current evaluation corpus. Each row is exact code
in the wild. The tool loads the pinned source excerpt, validates the source hash,
extracts the template, parser delimiter, parser regex, or parser control-flow
pattern from those bytes, and only then runs the checker. If the expected value
cannot be recovered from the source, the replay fails.
...
PromptABI agrees with all six GitHub-recorded bugs in the corpus. The Gemma4
...
\#43713. On a local CPU run, the complete evaluation command finishes in 0.61
...
Metric & Value \\
\midrule
Exact upstream production-code cases & 6 \\
Cases with expected bug found & 6 \\
Abstentions in counted cases & 0 \\
Model or provider calls & 0 \\
Total concrete witnesses/pattern agreements & 30 \\
Wall-clock runtime & 0.61s \\
\bottomrule
\end{tabular}
...
The evaluation is intentionally selective rather than padded. The search found
more recorded PromptABI-class failures than are counted here, but we did not add
cases merely because an issue contained a convincing payload. Non-production
regression fixtures are excluded from the evaluation claim. The claim is: for the
counted cases, PromptABI traverses the same public code that shipped upstream,
recovers the relevant boundary from that code, and produces concrete diagnostics
before inference.
